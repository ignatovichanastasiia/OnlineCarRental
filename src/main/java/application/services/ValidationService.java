...
/**
 * The ValidationService class provides static methods for validating various
 * inputs in the application. It includes validations for client data, email
 * format, rental dates (using LocalDate), and car availability.
 */
public class ValidationService {
	 ...
	 * <li>The identity number is 9 digits long (padding with leading zeros if
	 * necessary).</li>
	 * <li>The identity number passes the checksum validation using the standard
	 * algorithm.</li>
	 * </ul>
	 *
	 ...
	 * <li>The credit card number contains only numeric characters (after removing
	 * spaces).</li>
	 * <li>The credit card number passes the Luhn checksum algorithm.</li>
	 * </ul>
	 *
	 * @param creditCardNumber the credit card number to validate.
	 * @throws InvalidCreditCardNumberException if the credit card number is null,
	 *                                          empty, non-numeric, or invalid.
	 */
	 ...
	 * <li>The driver's license number matches the required format (numeric and 7 to
	 * 9 digits).</li>
	 * </ul>
	 *
	 * @param driversLicenseNumber the driver's license number to validate.
	 * @throws InvalidDriversLicenseNumberException if the driver's license number
	 *                                              is invalid.
	 */
	public static void validateDriversLicenseNumber(String driversLicenseNumber)
			throws InvalidDriversLicenseNumberException {
		...
		if (!sanitized.matches("\\d{7,9}")) {
			throw new InvalidDriversLicenseNumberException(
					"Driver's license number must be numeric and between 7 and 9 digits.");
		}
	}
...
//	}
	 ...
	 * <p>
	 * This method checks that the car is not null and its availability status is
	 * true.
	 *
	 ...
	 * @throws InvalidRentalDatesException if any of the date validations fail.
	 */
	public static void validateRentalDates(LocalDate startDate, LocalDate endDate)
